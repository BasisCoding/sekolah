...
if (!function_exists('menu_helper')) {
	function fetch_menu($data, $active=''){
		...
		foreach($data as $menu){
			...
			if (!empty($menu->sub)) {
				...
				$anchor = '<a class="nav-link '.$active.'" href="'.base_url($menu->link).'">
						<i class="'.$menu->icon.' '.$menu->color.'"></i>
						<span class="nav-link-text">'.$menu->nama_menu.'</span>
					</a>';
			}
	...
	function fetch_sub_menu($sub_menu, $active=''){
		$sub = "";
		foreach($sub_menu as $menu){
			...
			if (!empty($menu->sub)) {
				$anchor = '<a href="#'.$menu->link.'" class="nav-link '.$active.'" data-toggle="collapse" role="button" aria-expanded="false" aria-controls="'.$menu->link.'">'.$menu->nama_menu.'</a>';
			}else{
